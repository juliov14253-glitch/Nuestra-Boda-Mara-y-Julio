...
const INVITE_CONFIG = {
  ...
  dressCode: "Formal",
  welcome:
    "Nos encantará compartir este día contigo. Aquí encontrarás itinerario, ubicación y más detalles.",
  // Dirección principal para el mapa (usamos la parroquia)
  address: "Parroquia San José El Vidrio, Estado de México",
  coords: null, // si tienes lat/lng, pon { lat: ..., lng: ... }
  // Links directos proporcionados
  links: {
  ...
  // Galería (puedes agregar fotos a /assets/gallery y listarlas aquí)
  gallery: [ ],
  giftsMsg:
    "Tu presencia es nuestro mejor regalo. Si deseas hacernos un obsequio, aquí algunas opciones:",
  gifts: [
    "Sobre el día del evento"
  ],
  // Número WhatsApp con lada (sin +, sin espacios). Si está vacío, copiaremos el mensaje al portapapeles.
  whatsappNumber: ""
};
/* ==================================== */
...
(function main(){
  ...
  bindText("giftsMsg", INVITE_CONFIG.giftsMsg);
  ...
  // Regalos
  renderGifts(INVITE_CONFIG.gifts);
      ...
      rsvpBtn.addEventListener('click', async () => {
        ...
        if (phone){
          ...
          try{
            ...
            alert("Mensaje copiado. Pega en WhatsApp para confirmar.");
          }catch(e){
            prompt("Copia el siguiente mensaje y envíalo por WhatsApp:", msg);
          }
        }
      });
  }
...
function renderTimeline(items=[]){
  ...
  ul.innerHTML = items.map(it => `
    <li><time>${escapeHTML(it.time || "")}</time> <span class="label">${escapeHTML(it.title || "")}</span></li>
  `).join("");
}
...
function renderGallery(files=[]){
  ...
  wrap.innerHTML = files.map(f => `
    <figure><img src="assets/gallery/${encodeURIComponent(f)}" alt=""></figure>
  `).join("");
}

// Regalos
function renderGifts(items=[]){
  const ul = $('#giftList'); if(!ul) return;
  ul.innerHTML = items.map(x => `<li>${escapeHTML(x)}</li>`).join("");
}

// Mensaje WhatsApp RSVP
